...
MODEL_ID="$2"
ENGINE="$3"
RUNROOT="/tmp/flashbench_${MODEL_KEY}"
...
PYCFG
  ...
  mkdir -p "$STATE"
  # Auth only: avoids user plugins, sessions, MCPs, rules, and multi-GB state.
...
for N in $(seq -w 1 10); do
  ...
  if [ "$ENGINE" = commandcode ]; then
    ...
    bwrap --ro-bind / / --dev /dev --proc /proc \
      ...
      --ephemeral --dangerously-bypass-approvals-and-sandbox --disable apps --disable plugins --disable browser_use --disable computer_use --disable image_generation --disable skill_search --disable multi_agent --json "$(cat "$WORK/task.md")" >"$LOG" 2>&1 || RC=$?
  fi
  END=$(date +%s)
  printf '{"task":"%s","exit_code":%d,"elapsed_s":%d}\n' "$TASK" "$RC" "$((END-START))" > "$OUT/meta/$TASK.json"
  # Preserve candidate tree outside isolated run area for independent evaluation.
  cp -a "$WORK" "$OUT/$TASK"
